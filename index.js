...
const reactURI = "http://192.168.4.158";
...
app.get("/musixmatch", async (req, res) => {
  ...
  let url = encodeURI(
    `https://api.musixmatch.com/ws/1.1/track.search?apikey=${apiKey}&q_track=${trackName}&q_artist=${artistName}`
  );
  ...
  for (let currentTrack of tracks) {
    ...
    if (
      current.track_name.toLowerCase() === trackName.toLowerCase() &&
      current.artist_name.includes(artistName) &&
      current.has_lyrics === 1
    ) {
      song = current;
    }
  }
